...
| staging     | The `staging` deployment target is part of the CD pipeline. Latest staging content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-my_mlops_project-model | /staging-my_mlops_project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest prod content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-my_mlops_project-model    | /prod-my_mlops_project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the staging branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-my_mlops_project-model    | /test-my_mlops_project-experiment    |
...
You can open a PR (pull request) to modify ML code or the resource config against staging branch.
...
Upon merging a PR to the staging branch, the staging branch content will be deployed to the staging workspace with `staging` environment resource configurations.
...
```
Open a pull request to merge the pushed branch into the `staging` branch.
...
Once CI passes, merge the PR into the `staging` branch. This will deploy an initial set of Databricks resources to the staging workspace.
resources will be deployed to the prod workspace on pushing code to the `prod` branch.
...
After setting up CI/CD, PRs against the staging branch will trigger CI workflows to run unit tests, integration test and resource validation.
...
After merging a PR to the staging branch, continuous deployment automation will deploy the `staging` resources to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from staging to prod. Continuous deployment automation will deploy `prod` resources to the prod workspace.
