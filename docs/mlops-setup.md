...
Commit the current `README.md` file and other docs to the `staging` branch of the repo, to enable forking the repo:
```
...
* You must be a Databricks workspace admin in the staging and prod workspaces. 
  ...
  [staging workspace admin console](https://adb-1003894802230770.10.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-453519004944968.8.azuredatabricks.net#setting/accounts). 
...
The workflow will create a pull request with all the changes against your staging branch. Review and approve it to commit the files to deploy CI/CD for the project. 
...
After the pull request merges, pull the changes back into your local `staging`
branch:
...
Create and push a release branch called `prod` off of the `staging` branch of the repository:
```
...
Your production jobs (model training, batch inference) will pull ML code against this branch, while your staging jobs will pull ML code against the `staging` branch. Note that the `staging` branch will be the source of truth for ML resource configs and CI/CD workflows.

For future ML code changes, iterate against the `staging` branch and regularly deploy your ML code from staging to production by merging code changes from the `staging` branch into the `prod` branch.
